function deepCopy(obj) {
    if (Object.prototype.toString.call(obj) === '[object Array]') {
        var out = [], i = 0, len = obj.length; for (; i < len; i++) { out[i] = arguments.callee(obj[i]); }
        return out;
    }
    if (typeof obj === 'object') {
        var out = {}, i; for (i in obj) { out[i] = arguments.callee(obj[i]); }
        return out;
    }
    return obj;
}
function tinhdiem(acan, achi) {
    var canvas = document.getElementById('myCanvas'); var context = canvas.getContext('2d'); var diem = new Array(); var ct = new Array(); for (var i = 1; i < 5; i++) {
        ct[i] = new Array(); for (var j = 0; j < 4; j++)
            ct[i][j] = cantang[achi[i]][j]
    }; var dt = new Array(); for (var i = 1; i < 5; i++) {
        dt[i] = new Array(); for (var j = 0; j < 4; j++)
            dt[i][j] = cantang[achi[i]][j]
    }; for (var i = 1; i < 5; i++) {
        dt[i][0] = ct[i][0]; if (ct[i][0] == 1) { dt[i][1] = 30; }
        if (dt[i][0] == 2) { dt[i][1] = 21; dt[i][2] = 9; }
        if (dt[i][0] == 3) { dt[i][1] = 18; dt[i][2] = 9; dt[i][3] = 3; }
    }
...
function inracaccanbiendoi(ct, st, vitri) {
    for (var i = 1; i < 5; i++) { ansaotutru(can[ct[i][1]] + " ewfewfwe", i, vitri, 1); }
}
{
    function checktamhoi(x, y, z) {
        var points = [check(x + 1), check(y + 1), check(z + 1)]; points.sort(function (a, b) { return a - b }); for (var i = 0; i < 3; i++)
    ...
    function checktamhop(x, y, z) {
        var points = [check(x), check(y), check(z)]; points.sort(function (a, b) { return a - b }); for (var i = 0; i < 3; i++)
            points[i] = check(points[i] - i * 4); if (points[0] != points[1])
            return new Array(0, 0, 0, 0); else if (points[1] != points[2])
            return new Array(0, 0, 0, 0); else {
                var temp = points[1]; if (temp == 2)
                    temp = 10; if (temp == 3)
                temp = 7; return new Array(temp, temp, temp, temp);
        };
    }
    function checknhihop(x, y) {
        var points = [check(x), check(y)]; points.sort(function (a, b) { return a - b }); if ((check(x + y) != 3) || (x == y))
            return 0; else
            if ((x == 2) || (y == 2))
                return 5; else
                if ((x == 3) || (y == 3))
...
function draw() {
    var canvas = document.getElementById('myCanvas'); if (canvas.getContext) { var context = canvas.getContext('2d'); drawFractalTree(context); }
...
function drawLine(context, x1, y1, x2, y2, thickness) {
    context.fillStyle = '#000'; if (thickness > 6)
...
function thapthan(can1, can2) {
    var temp = 2 * (INT((can2 - 1) * 0.5) - INT((can1 - 1) * 0.5)); if (temp < 0)
        temp = temp + 10; if ((can2 - can1 + 10) % 2 != 0)
        temp = temp + 1; return temp;
}
function incantang(n, x, y, ccan1, than1, ccan2, than2, ccan3, than3) {
    var canvas = document.getElementById('myCanvas'); var context = canvas.getContext('2d'); if (n == 1) { g(ccan1); context.font = 'bold 12pt Arial'; context.fillText(can[ccan1], x, y); context.font = '11pt Arial'; context.fillText(than1, x, y + lineskip); context.fillText(ts[vongtruongsinhtutru(ccan1, achi[2])], x, y + 2 * lineskip); }
...
function nguhanhcan(x, y) {
    x = checkcan(x); y = checkcan(y); if (INT((x - 1) / 2) == INT((y - 1) / 2))
        return true; else
        return false;;
}
function convertcan2nguhanh(x) { x = check(x); return INT((x + 1) / 2); }
function anthansat(sao, chi) {
    var canvas = document.getElementById('myCanvas'); var context = canvas.getContext('2d'); for (var i = 1; i <= 4; i++) {
        if (achi[i] == chi) {
            if (thansat[i] < 4)
...
function ansaott(sao, i) {
    var canvas = document.getElementById('myCanvas'); var context = canvas.getContext('2d'); if (thansat[i] < 4)
        context.fillText(sao, (2 * i - 2) * 220 / 2 + 40, thansat[i] * lineskip + 800); else { context.fillText(sao, (2 * i - 1) * 220 / 2 + 40, (thansat[i] - 4) * lineskip + 800); }
    thansat[i]++;
}
...
function kinhduong(can) {
    var t = 0; switch (can) { case 1: t = 4; break; case 2: t = 3; break; case 3: t = 7; break; case 4: t = 6; break; case 5: t = 7; break; case 6: t = 6; break; case 7: t = 10; break; case 8: t = 9; break; case 9: t = 1; break; case 10: t = 12; break; }
    return t;
}
...
function quocanquynhan(can) {
    var t = 0; switch (can) { case 1: t = 11; break; case 2: t = 12; break; case 3: t = 2; break; case 4: t = 3; break; case 5: t = 2; break; case 6: t = 3; break; case 7: t = 5; break; case 8: t = 6; break; case 9: t = 8; break; case 10: t = 9; break; }
    return t;
}
function thieny(thang) {
    var t = 0; switch (check(thang)) { case 1: t = 2; break; case 2: t = 3; break; case 3: t = 4; break; case 4: t = 5; break; case 5: t = 6; break; case 6: t = 7; break; case 7: t = 8; break; case 8: t = 9; break; case 9: t = 10; break; case 10: t = 11; break; case 11: t = 12; break; case 12: t = 1; break; }
    return t;
}
function kimdu(can) {
    var t = 0; switch (can) { case 1: t = 5; break; case 2: t = 6; break; case 3: t = 8; break; case 4: t = 9; break; case 5: t = 8; break; case 6: t = 9; break; case 7: t = 11; break; case 8: t = 12; break; case 9: t = 2; break; case 10: t = 3; break; }
    return t;
}
function ductu(chi) {
    var t = 0; var u = 0; switch (chi % 4) { case 3: anthansatcan("Đức Quý Nhân", 3); anthansatcan("Đức Quý Nhân", 4); anthansatcan("Tú Quý Nhân", 5); anthansatcan("Tú Quý Nhân", 10); break; case 1: anthansatcan("Đức Quý Nhân", 9); anthansatcan("Đức Quý Nhân", 10); anthansatcan("Đức Quý Nhân", 5); anthansatcan("Đức Quý Nhân", 6); anthansatcan("Tú Quý Nhân", 3); anthansatcan("Tú Quý Nhân", 8); anthansatcan("Tú Quý Nhân", 1); anthansatcan("Tú Quý Nhân", 6); break; case 2: anthansatcan("Đức Quý Nhân", 7); anthansatcan("Đức Quý Nhân", 8); anthansatcan("Tú Quý Nhân", 2); anthansatcan("Tú Quý Nhân", 7); break; case 0: anthansatcan("Đức Quý Nhân", 1); anthansatcan("Đức Quý Nhân", 2); anthansatcan("Tú Quý Nhân", 4); anthansatcan("Tú Quý Nhân", 9); break; }
    return t;
}
function anloc(i) {
    var l; if (i == 1)
...
function coloan(a, b) {
    var coloan = new Array(2, 4, 8, 5, 5, 5, 9, 3); var coloanchi = new Array(6, 6, 12, 9, 3, 7, 1, 7)
    for (var i = 0; i < 4; i++)
        if (a == coloan[i])
...
function thienxa(a, b, j) {
    var thienxa = new Array(5, 1, 5, 1); var thienxachi = new Array(3, 7, 9, 1); for (var i = 0; i < 4; i++)
        if (a == thienxa[i])
            if (b == thienxachi[i])
...
function tuphe(a, b, j) {
    var tuphe = new Array(7, 8, 9, 10, 1, 2, 3, 4); var tuphechi = new Array(9, 10, 1, 12, 3, 4, 6, 7); for (var i = 0; i < 4; i++)
        if (a == tuphe[i])
            if (b == tuphechi[i])
...
function ankhoiviet(tcannam, ch) {
    var tk; var tv; if (tcannam == 1) { tk = 6; tv = 12; } else
...
function khoicanh(x, y) {
    var a = new Array(x, y); var nt = new Array(9, 5); var ct = new Array(7, 11); var cth = new Array(7, 5); var mt = new Array(5, 11); var canvas = document.getElementById('myCanvas'); var context = canvas.getContext('2d'); if (arraysAreIdentical(a, nt) == true) { context.fillText("Khôi Canh QN", (2 * 3 - 2) * w / 2 + 40, thansat[3] * lineskip + 800); thansat[3]++; }
...
function kimthan(x, y, n) {
    var a = new Array(x, y); var nt = new Array(2, 2); var ct = new Array(6, 6); var cth = new Array(10, 10); if (arraysAreIdentical(a, nt) == true) { context.fillText("Kim Thần", (2 * n - 2) * w / 2 + 40, thansat[3] * lineskip + 800); thansat[3]++; }
...
function locton(can) {
    var t = 0; switch (can) { case 1: t = 3; break; case 2: t = 4; break; case 3: t = 6; break; case 4: t = 7; break; case 5: t = 6; break; case 6: t = 7; break; case 7: t = 9; break; case 8: t = 10; break; case 9: t = 12; break; case 10: t = 1; break; }
    return t;
}
...
function vanxuongquynhan(can) {
    var t = 0; switch (can) { case 1: t = 6; break; case 2: t = 7; break; case 3: t = 6; break; case 4: t = 7; break; case 5: t = 9; break; case 6: t = 10; break; case 7: t = 12; break; case 8: t = 1; break; case 9: t = 3; break; case 10: t = 4; break; }
    return t;
}
function thaicuc(x) {
    var day; switch (x) { case 1: day = 1; break; case 2: day = 7; break; case 3: day = 10; break; case 4: day = 4; break; case 5: day = "tuquy"; break; case 6: day = "tuquy"; break; case 7: day = 3; break; case 8: day = 12; break; case 9: day = 6; break; case 10: day = 9; break; }
    return day;
}